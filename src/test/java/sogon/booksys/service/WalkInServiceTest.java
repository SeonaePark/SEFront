...
class WalkInServiceTest {
    ...
    void 예약_테스트(){
        ...
        Long walkInId = walkInService.walkIn(table.getId(), time, term, 4);

        //then
        WalkIn savedWalkIn = walkInService.findById(walkInId).get();
        assertEquals(savedWalkIn.getId(), walkInId);
    }
    ...
    void 예약취소_테스트(){
        ...
        tableRepository.save(table);
        Long walkInId = walkInService.walkIn(table.getId(), time, term, 4);
        WalkIn WalkIn = walkInService.findById(walkInId).get();

        walkInService.cancelWalkIn(walkInId);

        List<WalkIn> all = walkInService.findAll();
        //then
        assertFalse(all.contains(WalkIn));
    }
    ...
    void findByTable_테스트(){
        ...
        Long walkInId = walkInService.walkIn(table.getId(), time, term, 4);
        WalkIn WalkIn = walkInService.findById(walkInId).get();
        List<WalkIn> byTable = walkInService.findAllByTable(table);

        //then
        assertTrue(byTable.contains(WalkIn));
    }
    ...
    void 예약테이블_변경_테스트(){
        ...
        Long walkInId = walkInService.walkIn(table1.getId(), time, term, 4);
        Long updateId = walkInService.moveTable(walkInId, table2.getId());

        //then
        WalkIn preWalkIn = walkInService.findById(walkInId).get();
        WalkIn updateWalkIn = walkInService.findById(updateId).get();
        assertEquals(preWalkIn.getId(), updateWalkIn.getId());
        assertEquals(updateWalkIn.getTable().getSeats(), 5);
    }
    ...
    void 예약테이블_변경_같은시간_테스트(){
        ...
        tableRepository.save(table1);
        tableRepository.save(table2);
        Long walkInId = walkInService.walkIn(table1.getId(), time1, term, 4);
    ...
    void 예약_변경_테스트(){
        ...
        Long updateId = walkInService.updateWalkIn(walkInId, time2, term, 3);

        //then
        WalkIn WalkIn = walkInService.findById(updateId).get();
        assertEquals(WalkIn.getStartTime(), time2);
    }
